...
<p align="center">
  ...
  <br/>
    HR 담당자가 직원의 교육 현황과 미수료 상태를 관리할 수 있도록 만든 HR 교육 관리 프로젝트입니다. <br/>
    Codex CLI와 하위 에이전트를 개발 과정 전반에 활용했으며 <br/>
    1차 개발 이후 기존 UI와 데이터 처리 방식을 다시 검토하여 <br/>
    Project Skill과 라이브러리를 적용한 2차 개선을 진행했습니다.
  <br/>
...
1차 개발을 완료한 뒤 기존 프로젝트를 다시 검토하면서
...
직원의 교육 이수 상태가 변경되면 한 화면만 바뀌는 것이 아니라 다음 데이터가 함께 영향을 받습니다.
...
교육 이수 상태 변경이 성공하면 다음 Query를 함께 무효화하여 최신 데이터를 다시 조회하도록 구성했습니다.
...
검색 요청마다 ID를 부여하고 현재 요청의 ID와 일치하는 응답만 화면에 반영하도록 변경했습니다. <br/>
검색 초기화 시에도 요청 ID를 변경해 이미 진행 중이던 이전 요청이 이후 화면 상태에 영향을 주지 않도록 처리했습니다.
...
### [ 오류 발생 후 사용자가 작업을 이어갈 수 있도록 Retry 구조 개선 ]
...
새롭게 알게 된 Project SKill과 필요한 라이브러리를 추가하는 방식으로 진행하게되었습니다.
  
#### Project Skill
+ 사용한 가능한 Skill 모두 적용하기보다, 현재 프로젝트에서 실제로 개선할 문제와 연결되는지를 기준으로 선택했습니다.
+ `redesign-existing-projects` <br/>
